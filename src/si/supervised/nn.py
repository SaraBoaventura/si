...
class NN(Model):
    ...
    def fit_batch(self,dataset, batchs):
        X,y = dataset.getXy()
        if batchs > X.shape[0]:
        ...
        self.history = dict()
        for epoch in range(self.epochs):
            self.his_batch = np.zeros((1,batchs))
            for batch in range(batchs):
                output = X[int(batch * e):int((batch + 1) * e), ]
                for layer in self.layers:
                    output= layer.forward(output)
                error = self.loss_prime(y[int(batch * e):int((batch + 1) * e), ],output)
                for layer in reversed(self.layers):
                    error = layer.backward(error,self.lr)
                #calcule average error
                err = self.loss(y[int(batch * e):int((batch + 1) * e), ],output)
                self.his_batch[0,batch] = err
            self.history[epoch]= np.average(self.his_batch)
            if self.verbose:
